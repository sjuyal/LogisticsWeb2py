...
def history():
    ...
    prods=db((db.history_table.city==auth.user.place) & (db.history_table.levelno==auth.user.levelno) & (db.history_table.productid==db.product_info.product_id)).select()
    print prods
   #db(db.product_info.product_id.belongs(db.history_table.city==auth.user.place)).select()
   #prods=db(((db.product_info.source_place==auth.user.place) & (db.product_info.product_status!=1)) | ((db.product_info.destination_place==auth.user.place) & (db.product_info.product_status==5))).select(orderby=db.product_info.product_id,limitby=(0,10))
...
def show_history():
    ...
    db.product_info.source_place.writable = False
    ...
    form = SQLFORM(db.product_info, record,fields=["product_id","product_name","company_name","receiver_name","receiver_email","address","source_place","destination_place","product_status"])
